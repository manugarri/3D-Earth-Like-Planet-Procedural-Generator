...
def iterate(seed, rot, im_rgbref_base, normal_base, cloud_map, i, d1, d2):
    ...
    for i in range(res):
        for d1 in range(multi_sampling):
            for d2 in range(multi_sampling):
                im_rgbref_base, normal_base, cloud_map = iterate(
                    seed, rot, im_rgbref_base, normal_base, cloud_map, i, d1, d2
                )
                continue
                z2 = radius**2 - (x[i, :] + dx[d1, d2])**2 - (
                    y[i, :] + dy[d1, d2])**2
